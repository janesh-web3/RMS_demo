  ...
  try {
    ...
    for (const bill of bills) {
      ...
      for (const paymentMethod of bill.paymentMethods) {
        paymentBreakdown[paymentMethod.type as keyof typeof paymentBreakdown] +=
...
export const getOrderHistory = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({
      ...
      hasPrevPage: parseInt(offset as string) > 0
    });
  } catch (error) {
...
export const getPaymentAnalytics = async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const bill of bills) {
      ...
      for (const paymentMethod of bill.paymentMethods) {
        const type = paymentMethod.type as keyof typeof paymentAnalytics;
    ...
    res.json({
      ...
      mixedPaymentPercentage:
        totalBills > 0 ? (mixedPaymentBills / totalBills) * 100 : 0,
    });
  } catch (error) {
...
export const getCreditAnalytics = async (req: AuthRequest, res: Response) => {
  try {
    const { startDate, endDate } = req.query;
    ...
    recentCreditTransactions.sort((a, b) => 
...
export const getSalesReport = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const salesTrends = await Bill.aggregate([
...
export const exportReport = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (format === 'csv') {
      ...
      for (const bill of bills) {
        const customer = (bill.customerId as any)?.name || 'Walk-in';
        const paymentMethods = bill.paymentMethods.map(pm => `${pm.type}:${pm.amount}`).join(';');
        const items = bill.orders.flatMap((order: any) =>
          order.items.map((item: any) => `${item.itemId.name}(${item.quantity})`)
        ).join(';');
